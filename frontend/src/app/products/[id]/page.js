                      ...
                      try {
                        if (originalSrc.includes("localhost") || !originalSrc.includes("https://")) {
                          ...
                          if (originalSrc.includes('/media/')) {
                            ...
                            console.log("Trying fixed product URL:", fixedUrl);
                            e.target.src = fixedUrl;
                          } else {
                            throw new Error("Cannot fix product URL - no media path");
                          }
                        } else {
                          throw new Error("Using fallback product image");
                        }
                      } catch (err) {
                        console.log("Using fallback product image:", err.message);
                          ...
                          try {
                            if (originalSrc.includes("localhost") || !originalSrc.includes("https://")) {
                              ...
                              if (originalSrc.includes('/media/')) {
                                const mediaPath = originalSrc.split('/media/')[1];
                                const fixedUrl = `${MEDIA_BASE_URL}/media/${mediaPath}`;
                                console.log("Trying fixed related product URL:", fixedUrl);
                                e.target.src = fixedUrl;
                              } else {
                                throw new Error("Cannot fix related product URL - no media path");
                              }
                            } else {
                              throw new Error("Using fallback related product image");
                            }
                          } catch (err) {
                            console.log("Using fallback related product image:", err.message);
                            // Fallback to data URI
